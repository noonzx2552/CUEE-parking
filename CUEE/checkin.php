...
<head>
  ...
  <title>SmartPark — เริ่มจอด</title>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .btn{width:100%;background:linear-gradient(135deg,#7c4dff,#e040fb);color:#fff;border:none;border-radius:16px;padding:16px;font-family:'Inter',sans-serif;font-size:15px;font-weight:700;cursor:pointer;box-shadow:0 8px 20px rgba(124,77,255,.3);transition:all .2s;position:relative;overflow:hidden}
...
<body>
  ...
  <div class="card">
    <div class="logo-row">
      ...
      <div class="logo-title">SMART PARK</div>
    </div>
    <div class="sub">ระบบควบคุมทางเข้าอัจฉริยะ (Entrance)</div>

    <div class="scene">
      <div class="scene-bg"></div>
      <div class="road"><div class="road-line"></div><div class="road-edge"></div></div>
      <div class="status" id="status-txt">🔴 ไม้กั้น : ปิด</div>
      <div class="post"><div class="post-light" id="post-light"></div></div>
      <div class="arm-wrap" id="arm"><div class="arm"></div><div class="arm-tip"></div></div>
      
      <svg class="car" id="car" width="60" height="30" viewBox="0 0 100 50">
    ...
    <button class="btn" id="btn" onclick="startParking()">🅿️ เริ่มเข้าจอดรถ</button>
  </div>

  <div class="overlay" id="overlay">
    <div style="text-align:center;">
      <div class="spinner" style="margin:0 auto 20px;"></div>
      <div style="font-weight:600;color:#a78bfa;letter-spacing:1px;">กำลังเข้าสู่ระบบ...</div>
    </div>
  </div>
  ...
  <script>
    function startParking() {
      ...
      // 🚩 บันทึกเวลาเข้าประตูตรงนี้! (timestamp)
      const entranceTime = Date.now();
      localStorage.setItem('sp_entrance_time', entranceTime);
      
      // บันทึกเวลาแบบ string สำหรับแสดงผล
      const now = new Date();
      const timeStr = now.toLocaleTimeString('th-TH', {hour: '2-digit', minute: '2-digit'});
      localStorage.setItem('sp_entrance_time_display', timeStr);

      btn.disabled    = true;
      btn.textContent = '⏳ กำลังเปิดไม้กั้น...';

      setTimeout(() => { car.className = 'car at-gate'; }, 200);

      setTimeout(() => {
        arm.classList.add('open');
        postLight.classList.add('green');
        statusTxt.style.color = '#34d399';
        statusTxt.textContent = '🟢 ไม้กั้น : เปิดแล้ว!';
        btn.textContent       = '🚗 กำลังเข้า...';
      }, 1200);

      setTimeout(() => {
        car.className         = 'car entering';
        statusTxt.style.color = '#a78bfa';
        statusTxt.textContent = '✅ กำลังเข้าที่จอดรถ';
      }, 2400);

      setTimeout(() => { car.className = 'car passed'; }, 3600);
