...
export default function QrEntrancePage() {
  ...
  useEffect(() => {
    const qrData = window.location.origin + '/checkin'
  ...
  return (
    <>
      <style>{`
        ...
        .logo{font-family:'Orbitron',sans-serif;font-size:24px;font-weight:900;background:linear-gradient(90deg,#a78bfa,#fff,#34d399);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:8px}
        .sub{font-size:13px;color:#64748b;margin-bottom:24px}
        .qr-box{background:#fff;border-radius:16px;padding:20px;display:inline-block;margin-bottom:20px}
        .hint{font-size:13px;color:#a78bfa;font-weight:600}
        .hint2{font-size:11px;color:#475569;margin-top:6px}
      `}</style>
      <div className="card">
        <div className="logo">SMART PARK</div>
        <div className="sub">สแกน QR เพื่อเริ่มจอดรถ</div>
        <div className="qr-box">
          <div ref={qrRef} />
        </div>
        <div className="hint">📱 สแกนเพื่อเข้าระบบ</div>
